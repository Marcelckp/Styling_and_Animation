...
void main() {
  ...
  // Light Position which is moving in a circular motion around the scene over time
  vec3 lightPosition = vec3(-10.0 * cos(uTime * 1.5), 10.0, 10.0);

  vec3 ro = vec3(0.0, 0.0, 5.0);
  vec3 rd = normalize(vec3(uv, -1.0));

  // The raymarching algorithm will return the distance to the closest object in the scene from the camera to create the 3D effect 
  float d = raymarch(ro, rd);
  vec3 p = ro + rd * d;

  vec3 color = vec3(0.0);

  // If the distance is less than the maximum distance, calculate the normal and the light direction to get the diffuse lighting and shadows
  if(d<MAX_DIST) {
